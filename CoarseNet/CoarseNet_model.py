...
# sys.path.insert(1, "/PycharmProjects/MinutiaeNot/FineNet")
...
def CoarseNetmodel(input_shape=(400, 400, 1), weights_path=None, mode='train'):
    ...
    if mode == 'deploy':
        model = Model(inputs=[img_input, ],
                      outputs=[enh_img, enh_img_imag, enh_img_real, ori_out_1, ori_out_2, seg_out, mnt_o_out, mnt_w_out,
                               mnt_h_out, mnt_s_out])
    else:
        model = Model(inputs=[img_input, ],
...
def train(input_shape=(400, 400), train_set=None,
    ...
    for epoch in range(1000):
        ...
        for i, train in enumerate(
                load_data((img_name, folder_name, img_size), tra_ori_model, rand=True, aug=0.7, batch_size=batch_size)):
        ...
        if epoch % 10 == 0:
            ...
            for folder in test_set:
                precision_test, recall_test, F1_test, precision_test_location, recall_test_location, F1_test_location = evaluate_training(
                    savedir, [folder, ], logging=logging)
            ...
            summary = tf.Summary(value=[tf.Summary.Value(tag="Precision", simple_value=precision_test),
                                        ...
                                        tf.Summary.Value(tag="Location Precision",
...
def evaluate_training(model_dir, test_set, logging=None, FineNet_path=None):
    ...
    return ave_prf_nms[0], ave_prf_nms[1], ave_prf_nms[2], ave_prf_nms_location[0], ave_prf_nms_location[1], \
...
def deploy_with_GT(deploy_set, output_dir, model_path, FineNet_path=None, set_name=None):
    ...
    for i, test in enumerate(
        ...
        draw_minutiae_overlay_with_score(image, mnt_nms, mnt_gt[:, :3],
                                         "%s/%s/%s_minu.jpg" % (output_dir, set_name, img_name[i]), saveimage=True)
...
def inference(deploy_set, output_dir, model_path, FineNet_path=None, set_name=None, file_ext='.bmp',
              isHavingFineNet=False):
    if set_name is None:
        set_name = deploy_set.split('/')[-2]
    ...
    for i in range(0, len(img_name)):
        ...
        time_afterpost = time()
        mnt_writer(mnt_nms, img_name[i], img_size, "%s/%s/mnt_results/%s.mnt" % (output_dir, set_name, img_name[i]))
        draw_minutiae(original_image, mnt_nms, "%s/%s/%s_minu.jpg" % (output_dir, set_name, img_name[i]),
                      saveimage=True)
