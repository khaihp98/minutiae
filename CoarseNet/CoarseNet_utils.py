...
def sub_load_data(data, img_size, aug):
    ...
    img = cv2.imread(dataset + 'img_files/' + img_name + '.bmp', 0)

    if os.path.isfile(dataset + "seg_files/" + img_name + '.bmp'):
        seg = cv2.imread(dataset + 'seg_files/' + img_name + '.bmp', 0)
    else:
        seg = np.ones_like(img)
    
    if os.path.isfile(dataset + "ori_files/" + img_name + ".jpg"):
        ali = cv2.imread(dataset + 'ori_files/' + img_name + '.jpg', 0)
    else:
...
def ori_acc_delta_k(y_true, y_pred, k=10, max_delta=180):
    ...
    label_seg = K.cast(K.greater(label_seg, 0), tf.float32)
    # get pred angle
    ...
    acc = K.less_equal(K.minimum(angle_delta, max_delta - angle_delta), k)
...
def seg_acc_pos(y_true, y_pred):
    y_true = tf.where(K.less(y_true, 0.0), K.zeros_like(y_true), y_true)
...
def atan2(y_x):
    ...
    angle = tf.where(K.greater(x, 0.0), atan, K.zeros_like(x))
    angle = tf.where(tf.math.logical_and(K.less(x, 0.0), K.greater_equal(y, 0.0)), atan + np.pi, angle)
    angle = tf.where(tf.math.logical_and(K.less(x, 0.0), K.less(y, 0.0)), atan - np.pi, angle)
    return angle
...
def orientation(image, stride=8, window=17):
    with K.name_scope('orientation'):
        ...
        with K.name_scope('sobel_gradient'):
        ...
        with K.name_scope('gaussian_filter'):
            phi_x = tf.nn.conv2d(K.cos(theta), gaussian, strides=[1, 1, 1, 1], padding='SAME', name='gaussian_x')
            phi_y = tf.nn.conv2d(K.sin(theta), gaussian, strides=[1, 1, 1, 1], padding='SAME', name='gaussian_y')
...
def get_maximum_img_size_and_names(dataset, sample_rate=None, max_size=None, extention=None):
    # if isinstance(dataset, np.basestring):
    #     dataset = [dataset]
    ...
    for folder, rate in zip(dataset, sample_rate):
        _, img_name_t = get_files_in_folder(folder + '*', extention)
        # print(img_name_t)
        
        img_name.extend(img_name_t.tolist() * rate)
        folder_name.extend([folder] * img_name_t.shape[0] * rate)

        img_size.append(np.array(cv2.imread(folder + img_name_t[0] + extention, 0).shape))
