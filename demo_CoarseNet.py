...
inference_set = ['Dataset/CoarseNet_test/img_files/test/']
# inference_set = ["Dataset/CoarseNet_test/img_files/latent/", "Dataset/CoarseNet_test/img_files/examplar/"]  
...
for i, deploy_set in enumerate(inference_set):
    ...
    # Read image and GT
    img_name, folder_name, img_size = get_maximum_img_size_and_names([deploy_set], extention=extention)
    # print(img_name)
    ...
    for i in range(0, len(img_name)):
        ...
        image = cv2.imread(deploy_set + img_name[i] + extention, 0)# / 255.0
        ...
        seg_mask = np.where(final_mask < 1, final_mask, 255)
        ...
        cv2.imwrite("%s/%s/seg_results/%s_seg.jpg" % (output_dir, set_name, img_name[i]), seg_mask)
        end = time()
        logging.info("Predict time: %ds" %(end-start))
